...
<section id="app-block" style="display:none" class="block">
    <div class="d-flex justify-content-between">
        <div class="text-xs-center">
            <h5 class="electric-title mb-0 text-md-larger text-light"><?php echo _('HOUSE USE') ?></h5>
        ...
        <div class="text-xs-center">
            <h5 class="electric-title mb-0 text-md-larger text-light"><?php echo _('SOLAR GENERATING') ?></h5>
    ...
    <div class="d-flex justify-content-between">
        <div class="text-xs-center">
            ...
            <h2 class="power-value display-md-3 display-lg-2 my-0 text-quaternary"><span class="battery_charge_discharge">-</span></h2>
        </div>
        <div class="text-xs-center">
        ...
        <div class="text-xs-center">
            <h5 class="electric-title mb-0 text-md-larger text-light"><?php echo _('CHARGING POWER') ?></h5>
            <h2 class="power-value display-md-3 display-lg-2 my-0 text-quaternary"><span class="battery_power">-</span>W</h2>
        </div>
    </div>
...
config.app = {
    ...
    "use":{"type":"feed", "autoname":"import", "engine":"5", "description":"Building consumption in watts (not including battery charging)"},
    ...
    "battery_power":{"optional":true, "type":"feed", "autoname":"battery_charge", "engine":"5", "description":"Battery Power when charging W"},
...
function livefn()
{
    ...
    //battery is charging is positive - negative is discharging after with solar
    if(solar_now > 0 && battery_charge_now > 0 ){ //battery is charging 
       houseUse = solar_now - battery_charge_now;  
    }else if(solar_now > 0 && battery_discharge_now < 0){ //house is using solar and battery
       houseUse = solar_now + (battery_discharge_now * -1);    
    }else if(solar_now <= 0 && battery_discharge_now < 0){ //house is using battery/discharging
       houseUse = battery_discharge_now * -1;    
    }else if(use_now < 0 && battery_charge_now > 0){ //house is charging battery from grid - importing
       houseUse = (use_now * -1) + battery_charge_now;    
    }else if(battery_charge_now == 0 && battery_discharge_now == 0 && solar_now > 0){ //house is charging battery from grid - importing
       houseUse = solar_now;    
    }


    //use_now is power1 = import -- import is negative - export = positive -- need to add battery charge/discharge
    if(use_now >= 0 && houseUse > 0){ //house is using solar and exporting
    	houseUse = houseUse - use_now
    }else if(use_now <= 0 && houseUse > 0){ // house is importing and using solar
  	    houseUse = houseUse + (use_now  * -1)
    }else if(houseUse  < 0 && use_now <= 0 ){ // house is importing and using battery
  	    houseUse = (houseUse + use_now) * -1
    } else if(houseUse == 0 && use_now <= 0 ){ // house is importing and using battery
  	    houseUse =  use_now * -1
    }   
    ...
    if (autoupdate) {
        ...
        timeseries.append("battery_power",updatetime,battery_charge_now + battery_discharge_now);
    ...
    //if (battery_charge_now10) battery_charge_now = 0;
    ...
    $(".battery_power").html(battery_charge_now + battery_discharge_now);
    ...
    if (net_battery_charge>0) {
        ...
        $(".battery_charge_discharge").html("<span style='color:#2ed52e'>"+ net_battery_charge+"W</span>");
        ...
        $(".battery_charge_discharge").html("<span style='color:#d52e2e'>"+-net_battery_charge+"W</span>");
    } else {
...
function load_powergraph() {
    ...
    if (reload) {
        ...
        timeseries.load("battery_power",feed.getdata(config.app.battery_charge.value,view.start,view.end,view.interval));
    ...
    for (var z=0; z<timeseries.length("solar"); z++) {
        ...
        if ((time-last_solar)<timeout && (time-last_use)<timeout && (time-last_charge)<timeout && (time-last_discharge)<timeout) {
	        ...
	        //battery is charging is positive - negative is discharging after with solar
    	    if(solar_now > 0 && battery_charge_now > 0 && use_now > 0){ //battery is charging 
               solar_direct = solar_now - battery_charge_now - (use_now * -1);  
            }else if(solar_now > 0 && battery_discharge_now < 0){ //house is using solar and battery
               solar_direct = solar_now ;    
            }else if(use_now < 0 && solar_now > 0){ //house is using solar and from grid - importing
              solar_direct = solar_now;    
            }
            ...
            var excess = 0;
            var unmet = 0;
            if (balance > 0) { //use_now negative means importing
                excess = balance
            } else {
                unmet = -1*balance;
          ...
          //  if (houseUse < 0 ){
           //     alert("1:" + use_now + " = " + solar_now + ": " + houseUse);
           // }
            ...
            if (use_now < 0 ){
                houseUse =  use_now * -1;
                use_now = houseUse;
             //   alert("2:" + use_now + " = " + solar_now + ": " + houseUse);
            }
	        ...
	        total_battery_discharge_kwh += (battery_discharge_now*interval*-1)/(1000*3600);
            use_data.push([time,use_now]);
            solar_data.push([time,solar_now]);
            battery_charge_data.push([time,battery_charge_now]);
            battery_discharge_data.push([time,battery_discharge_now*-1]);
            battery_soc_data.push([time,battery_soc_now]);

	        battery_power_data.push([time,battery_charge_now]);
	        ...
	        battery_power_data.push([time,null]);
        }
    ...
    var total_import_direct_kwh = total_import_kwh;
    var total_import_for_battery_kwh = total_battery_discharge_kwh;
    var total_battery_charge_from_solar_kwh = total_battery_charge_kwh;
    var total_solar_export_kwh = total_solar_export_kwh;
